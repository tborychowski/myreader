...
class Sources extends BaseController {
	...
	public function get() {
		...
		foreach ($sources as $src) {
			$srcs[] = [
				...
				'tag' => $src['tags'],
				'title' => $src['title'],
				'icon' => $itemsDao->getLastIcon($src['id']),
				'unread' => $itemsDao->numberOfUnreadForSource($src['id']),
			];
		}
		header('Content-type: application/json');
		echo $this->view->jsonSuccess($srcs);
	}
	...
	public function params() {
		...
		if($this->view->spout===false) $this->view->error('invalid spout type given');

		if($this->view->spout->params!==false) echo $this->view->render('templates/source_params.phtml');
	}
	...
	public function remove() {
		...
		if (!$sourceDao->isValid('id', $id)) $this->view->error('invalid id given');
	...
	public function write() {
		...
		if(!isset($data['title'])) $this->view->jsonError(array('title' => 'no data for title given'));
		if(!isset($data['spout'])) $this->view->jsonError(array('spout' => 'no data for spout given'));
		...
		if($validation!==true) $this->view->error( json_encode($validation) );

		// add/edit source
		$id = \F3::get('PARAMS["id"]');

		if (!$sourcesDao->isValid('id', $id)) $id = $sourcesDao->add($title, $tags, $spout, $data);
		else $sourcesDao->edit($id, $title, $tags, $spout, $data);

		// autocolor tags
		$tagsDao = new \daos\Tags();
		$tags = explode(",",$tags);
		foreach($tags as $tag) $tagsDao->autocolorTag(trim($tag));
	...
	public function renderSources($sources) {
		...
		foreach($sources as $source) {
			...
			$this->view->icon = $itemsDao->getLastIcon($source['id']);
